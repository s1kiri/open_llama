...
class Config:
    ...
    #
    # sbercredit https://arxiv.org/pdf/2305.13245.pdf
...
apple_slm = [
        dict(
        org="AIBSNLP-research",
        name="tiny_LLaMA_apple_upgrade_1.1b",
        block_size=2048,
        vocab_size=32000,
        padding_multiple=64,
        n_layer=22,
        n_head=12,      #TODO: это минимальное количество голов учитывая что скейлится колиество от глубины
        # mean_heads=32,  #TODO: это среднее количество голов учитывая что скейлится колиество от глубины(ллама инит дефолт)
        n_embd=2048,
        rotary_percentage=1.0,
        parallel_residual=False,
        bias=False,
        _norm_class="FusedRMSNorm",
        norm_eps=1e-8, #Llama 2 use 1e-5. Llama 1 use 1e-6
        _mlp_class="LLaMAMLP",
        intermediate_size=2048,      #TODO: это минимальный сайз FFN учитывая что скейлится колиество от глубины
        # mean_intermediate_size=5632, #TODO: это средний сайз FFN учитывая что скейлится колиество от глубины(ллама инит дефолт)
        n_query_groups=4, 
        a_min=1, 
        a_max=5,
        b_min=1,
