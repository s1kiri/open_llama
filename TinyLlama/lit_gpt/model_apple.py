...
class GPT(nn.Module):
    ...
    def forward(
        ...
        if use_kv_cache:
            ...
            sin = sin[:T]
            mask = None
#alias unzip_all=for z in *.zip; do unzip -O "utf-8 "$z -d "${z%.*}"; done;
    ...
    def build_rope_cache(self, idx: torch.Tensor) -> RoPECache:
        return build_rope_cache(
            seq_len=self.config.block_size,    # TODO self.config.head_size
            n_elem=int(self.config.rotary_percentage * self.config.head_size), # сомнительно и не понятно какой тут headsize взять) 
    ...
    def build_kv_caches(self, idx: torch.Tensor, max_seq_length: int, rope_cache_length: int) -> List[KVCache]:
        kv_cache_list = []
        for i, block in enumerate(self.transformer.h):
            head_size_at_layer = block.attn.config.head_size
            B = idx.size(0)
            ...
            k_cache_shape = (
                ...
                max_seq_length,
                heads,              #TODO                                                            #TODO
                rope_cache_length + head_size_at_layer - int(self.config.rotary_percentage * head_size_at_layer),  
            )
            v_cache_shape = (B, max_seq_length, heads, head_size_at_layer)
            device = idx.device
...
class Block(nn.Module):
    def __init__(self, config: Config, index) -> None:
        ...
        self.attn = CausalSelfAttention(config, index) #TODO
        self.index = index
        if not config.shared_attention_norm:
            self.norm_2 = config.norm_class(config.n_embd, eps=config.norm_eps)
        self.mlp = config.mlp_class(config, index) #TODO
        self.config = config
    def forward(
        self,
        x: torch.Tensor,
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: Config, index: int) -> None:
        super().__init__()
        config.n_head = self.get_i_th_size(config.a_min, config.a_max, index, config.n_layer)*config.n_head
        config.head_size = config.n_embd//config.n_head
        shape = (config.n_head + 2 * config.n_query_groups) * (config.n_embd//config.n_head)
        ...
        # TODO: дописать аттеншен всё перепроверить и дописать линеары.

    def get_i_th_size(self, min, max, index, n_layer):
        return min + ((max - min) * index) / (n_layer - 1)
...
class GptNeoxMLP(nn.Module):
    def __init__(self, config: Config, index: int) -> None:
        super().__init__()
        config.intermediate_size = self.get_i_th_size(config.b_min, config.b_max, 
                                                      index, config.n_layer) * config.intermediate_size
